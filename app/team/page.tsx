...
export default function TeamPage() {
  ...
  useEffect(function() {
    getDocs(collection(db, "players")).then(function(snap) {
  ...
  useEffect(function() {
    ...
    getDocs(
      ...
      const teams = snap.docs.map(function(d) {
        return { id: d.id, ...d.data() } as GWTeam;
      });
      setGwTeams(teams);
      setLoading(false);
    });
  ...
  const currentTeam = gwTeams.find(function(t) { return t.gameweek === selectedGW; });
  const availableGWs = gwTeams
    .filter(function(t) { return t.gameweek <= CURRENT_GW; })
    .map(function(t) { return t.gameweek; })
    .sort(function(a, b) { return b - a; });
  const playerIds = currentTeam
    ? [currentTeam.player1, currentTeam.player2, currentTeam.player3, currentTeam.player4]
    : [];

  const getPlayer = function(id: string) { return players[id]; };
  const isCaptain = function(id: string) { return currentTeam?.captain === id; };
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            {availableGWs.map(function(gw) {
              return (
                <button
                  key={gw}
                  onClick={function() { setSelectedGW(gw); }}
                  style={{
        ...
        {!currentTeam ? (
          ...
          <div>
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "1rem 1.5rem", marginBottom: "1.5rem", display: "flex", gap: "2rem", flexWrap: "wrap" }}>
              ...
              <div>
                <div style={{ color: "var(--text-muted)", fontSize: "0.75rem" }}>Transfer Penalty</div>
                ...
                return (
                  <div key={i} style={{ background: "var(--surface)", border: "1px solid " + (isCaptain(pid) ? "var(--blue)" : "var(--border)"), borderRadius: "10px", padding: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontSize: "0.85rem", color: "var(--accent)", marginTop: "0.2rem", fontWeight: 600 }}>{p?.points ?? 0} pts this GW</div>
                    </div>
                    {isCaptain(pid) && (
            ...
            {currentTeam.sub && (
              <div>
                <div style={{ color: "var(--text-muted)", fontSize: "0.75rem", letterSpacing: "1px", textTransform: "uppercase", marginBottom: "0.4rem" }}>Substitute</div>
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontWeight: 600 }}>{getPlayer(currentTeam.sub)?.name ?? currentTeam.sub}</div>
