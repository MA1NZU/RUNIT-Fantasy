...
function PlayerCard({ player, points, isCaptain, isSub, onClick }: { player: Player; points: number; isCaptain?: boolean; isSub?: boolean; onClick?: () => void }) {
...
function StatsModal({ player, stats, isCaptain, onClose }: { player: Player, stats: any, isCaptain: boolean, onClose: () => void }) {
  const s = (key: string) => Number(stats[key] || 0);
  
  const getBreakdown = () => {
    let rows: { label: string; val: any; pts: number }[] = [];
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }} onClick={onClose}>
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "20px", width: "100%", maxWidth: "450px", padding: "2rem", position: "relative" }} onClick={e => e.stopPropagation()}>
        <button onClick={onClose} style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", color: "#fff", fontSize: "1.2rem", cursor: "pointer" }}>✕</button>
        
        <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.5rem" }}>
          <div style={{ width: "60px", height: "60px", borderRadius: "10px", overflow: "hidden", background: "#222" }}>
             {player.image ? <img src={player.image} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : null}
          </div>
          <div>
            <h2 style={{ fontSize: "1.2rem", fontWeight: 800 }}>{player.name}</h2>
            <div style={{ fontSize: "0.75rem", color: "var(--accent)", fontWeight: 700 }}>{player.game.toUpperCase()} STATISTICS</div>
          </div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 60px", fontSize: "0.7rem", color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase", padding: "0 0.5rem" }}>
            <span>Statistic</span>
          ...
          <div style={{ maxHeight: "300px", overflowY: "auto", display: "flex", flexDirection: "column" }}>
            {rows.map((r, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px 60px", padding: "0.75rem 0.5rem", borderBottom: "1px solid var(--border)", fontSize: "0.9rem" }}>
                <span style={{ fontWeight: 600 }}>{r.label}</span>
                <span style={{ textAlign: "right", color: "var(--text-muted)" }}>{r.val.toLocaleString()}</span>
                <span style={{ textAlign: "right", fontWeight: 700, color: r.pts >= 0 ? "var(--green)" : "var(--red)" }}>
                  {r.pts > 0 ? `+${r.pts}` : r.pts}
                </span>
              </div>
            ))}
          ...
          <div style={{ marginTop: "1rem", padding: "1rem", background: "rgba(255,255,255,0.03)", borderRadius: "12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
             <div style={{ fontSize: "0.8rem", fontWeight: 700 }}>{isCaptain ? "Total GW Points (Doubled)" : "Total GW Points"}</div>
             <div style={{ fontSize: "1.2rem", fontWeight: 800, color: "var(--accent)" }}>{isCaptain ? totalRaw * 2 : totalRaw} pts</div>
          </div>
        </div>
      </div>
    </div>
...
function TeamContent() {
    ...
    const loadStats = async () => {
      try {
        ...
        statsSnap.docs.forEach(d => { 
          const data = d.data();
          if (data.Title) sMap[data.Title] = data; 
        });
        setMatchStats(sMap);
      } catch (err) { console.error(err); }
    };
  ...
  const getPoints = (id: string) => {
    const p = players[id];
    if (!p) return 0;
    return matchStats[p.name]?.gwPoints ?? 0;
  }
